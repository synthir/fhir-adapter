...
public class FHIRUtil {
	...
	public static String createFHIRResourceOnFHIRServer(String createResourceRequestUrl, String accessToken,
			String resourceJsonStr, String fhirServerBaseUrl, String tenantID, String clientID, String clientSecret) {
		...
		if (httpStatusCode != null) {
			if (httpStatusCode.toString().equalsIgnoreCase("401 UNAUTHORIZED")) {
				...
				accessToken = getAccessTokenFromAzureActiveDirectory(tenantID, fhirServerBaseUrl, clientID,
						clientSecret);
				createFHIRResourceOnFHIRServer(createResourceRequestUrl, accessToken, resourceJsonStr,
	 ...
	 */
	@SuppressWarnings("null")
	public static String getFHIRResourceFromFHIRServer(String getResourceRequestUrl, String accessToken,
			String fhirServerBaseUrl, String tenantID, String clientID, String clientSecret) {

		String responseBody = "";
		HttpStatusCode httpStatusCode = null;

		if (!(BasicUtil.isStringBlank(getResourceRequestUrl) || BasicUtil.isStringEmpty(getResourceRequestUrl)
				|| BasicUtil.isStringNull(getResourceRequestUrl))) {
			...
			ResponseEntity<String> response = null;
			try {
				response = restTemplate.exchange(new URI(getResourceRequestUrl), HttpMethod.GET, httpReqEntity,
						String.class);
			} catch (HttpClientErrorException | HttpServerErrorException httpClientOrServerExc) {
				httpStatusCode = httpClientOrServerExc.getStatusCode();
			...
			if (httpStatusCode != null) {
				if (httpStatusCode.toString().equalsIgnoreCase("401 UNAUTHORIZED")) {
					...
					accessToken = getAccessTokenFromAzureActiveDirectory(tenantID, fhirServerBaseUrl, clientID,
							clientSecret);
					getFHIRResourceFromFHIRServer(getResourceRequestUrl, accessToken, fhirServerBaseUrl, tenantID,
	...
	@SuppressWarnings("null")
	public static String getFHIRResourceFromFHIRServerWithQueryParams(String requestUrl, String queryParam,
			String accessToken, String fhirServerBaseUrl, String tenantID, String clientID, String clientSecret) {

		String responseBody = "";
		HttpStatusCode httpStatusCode = null;

		if (!(BasicUtil.isStringBlank(requestUrl) || BasicUtil.isStringEmpty(requestUrl)
				|| BasicUtil.isStringNull(requestUrl))) {
			HttpHeaders httpHeaders = new HttpHeaders();
			httpHeaders.setContentType(MediaType.APPLICATION_JSON);
			httpHeaders.set("Authorization", "Bearer " + accessToken);
			...
			if (httpStatusCode != null) {
				if (httpStatusCode.toString().equalsIgnoreCase("401 UNAUTHORIZED")) {
					...
					accessToken = getAccessTokenFromAzureActiveDirectory(tenantID, fhirServerBaseUrl, clientID,
							clientSecret);
					getFHIRResourceFromFHIRServerWithQueryParams(requestUrl, queryParam, accessToken, fhirServerBaseUrl,
	...
	public static String getFHIRResourceUrlFromResourceBody(String responseBody) {
		...
		if (!(BasicUtil.isStringBlank(responseBody) || BasicUtil.isStringEmpty(responseBody)
				|| BasicUtil.isStringNull(responseBody))) {
	...
	public static Object getResourceObjectFromResponse(String jsonResponseString) {
		...
		try {
			...
			if (jsonNodeEntryObj != null) {
				List<FHIRResourceResponse> fhirResourceResponseList = objectMapper
						.readValue(jsonNodeEntryObj.toString(), new TypeReference<List<FHIRResourceResponse>>() {
						});
	...
	public static String getAccessTokenFromAzureActiveDirectory(String tenantID, String fhirServerBaseUrl,
			String clientID, String clientSecret) {

		String accessToken = "";

		String getRequestUrl = APIConstants.AZURE_ACTIVE_DIRECTORY_OAUTH_BASE_URL + APIConstants.REQUEST_SEPARATOR
				+ tenantID + APIConstants.REQUEST_SEPARATOR + APIConstants.AZURE_ACTIVE_DIRECTORY_OAUTH_URL_PARAMS;
		...
		HttpEntity<MultiValueMap<String, String>> httpReqEntity = new HttpEntity<MultiValueMap<String, String>>(body,
				httpHeaders);

		RestTemplate restTemplate = new RestTemplate();
		@SuppressWarnings("null")
		ResponseEntity<String> response = restTemplate.exchange(getRequestUrl, HttpMethod.POST, httpReqEntity,
				String.class);
